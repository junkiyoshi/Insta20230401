...
void ofApp::draw() {
	...
	for (int v = 0; v < 360; v += v_span) {
		...
		for (int u = 0; u < 360; u += u_span) {
			...
			auto noise_value = ofNoise(glm::vec4(this->make_point(R, 10, u, v) * 0.085, ofGetFrameNum() * 0.015));
			auto param = noise_value > 0.5 ? noise_value < 0.65 ? ofMap(noise_value, 0.5, 0.65, 1, 0) : 0 : 1;
